...
map2.on('style.load', () => {
  ...
  fetchJSON('data/bioregions.geojson').then((dataGeo) => {
...
map.on('load', function () {
  ...
  fetchJSON('data/biography.geojson').then((dataP) => {
  ...
  map.on('mousemove', 'bioPoly', (e) => {
    map.getCanvas().style.cursor = 'pointer';
    if (e.features.length > 0) {
      if (hoveredStateId !== null) {
        map.setFeatureState(
          { source: 'bioJson', id: hoveredStateId },
          { hover: false }
        );
      }
    ...
    popup
      ...
      .setHTML(
        '<h6> Year: ' +
          Math.trunc(e.lngLat.lng * 100000 * 33.128) +
          '<br>' +
          e.features[0].properties.name +
          ' (b. ' +
          e.features[0].properties.birth +
          ' - d. ' +
  ...
  map.on('click', 'bioPoly', (e) => {
    var placename = e.features[0].properties.region;
    console.log(placename);
    jumpy(placename);
    map2.getBounds('bioPoly');
  });
  function jumpy(x) {
    fetch('data/bioregions.geojson')
      .then((response) => response.json())
      .then((data) => {
        //
        data.features.forEach((item) => {
          if (item.properties.region === x) {
...
function geoFilters2(f) {
  map.getSource('bioJson').setData(f);
  map.setLayoutProperty('bioPolyLines', 'visibility', 'visible');
  var bbox = turf.bbox(f);
  map.fitBounds(bbox, {
    padding: {
      top: 10,
...
function populateList(d) {
  ...
  d.features.forEach((item) => {
    list1.push(item.properties.profession);
  ...
  .then((data) => {
    $('.livello')
      ...
      .change(function (event) {
        if (event.target.id === 'primoLivello') {
          ...
          data.features.forEach((item) => {
            if (event.target.value === '') {
              ...
              if (item.properties.profession === event.target.value) {
                list2.push(item.properties.name);
                filteredGeojson.features.push(item);
              }
            }
        ...
        geoFilters2(filteredGeojson);

        // if (!$('#primoLivello').has('option').html() == '') {
        //   geoFilters2(filteredGeojson);
        // } else if ($('#primoLivello').has('option').html() == '') {
        //   map.setLayoutProperty('bioPolyLines', 'visibility', 'none');
        // }
      });
  });
